...
- Dertien tests van de echte serverhandlers met gesimuleerde aanbieders geslaagd: onafhankelijke push/mailkanalen, herhaalpogingen, ingetrokken deelrechten, geverifieerde terugbetalingen, paginering, accountcontrole, storingsgedrag en veilige SMTP-foutcodes zonder wachtwoorden/providerreacties. Dit bewijst geen echte mailbox-, push- of storebezorging.
...
1. **Supportmail:** `SUPPORT_SMTP_JSON` is op 22 september opgeslagen en wordt door de live worker gelezen. De echte SMTP-aanmelding wordt door STRATO geweigerd (`MAIL_EAUTH`): controleer het exacte mailboxadres en het mailboxwachtwoord in de beveiligde beheeromgeving. Na correctie de gepauzeerde testopdracht hervatten en ontvangst bevestigen. De bestaande Auth-SMTP is geen automatisch gedeelde Edge-secret. Standaard ontvangt `info@paskluis.nl` medewerkersmeldingen; `SUPPORT_INBOX_EMAIL` kan dit wijzigen. Voer wachtwoorden en sleutels uitsluitend in de beveiligde beheeromgeving in.
