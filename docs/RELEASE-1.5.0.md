...
- Databasewachtrij voor deel- en supportmeldingen, aparte push- en mailstatus, herhaalpogingen, generieke melding zonder kaartcode/PIN/gesprekstekst. Worker en automatische herhaalpogingen zijn actief. De aparte support-SMTP is opgeslagen. Het aanvankelijk onjuiste .nl-mailboxadres gaf `MAIL_EAUTH`; na eigenaarcorrectie gebruikt `SUPPORT_MAILBOX=info@paskluis.com` de bestaande opgeslagen SMTP-password met de juiste gebruikersnaam en afzender. De test is opnieuw gestart; de nieuwe verzendstatus kon door een vastgelopen browsercontrole nog niet worden opgehaald. Echte push-/mailbezorging moet nog succesvol worden bevestigd.
...
- Live smokecontrole: meldingsworker antwoordt HTTP 200 met lege wachtrij; onbevoegde account-/aankoop-/deelverzoeken worden geweigerd. De locatiefunctie accepteert de publieke app-sleutel en valideert de invoer. Geen echte klantberichten verstuurd tijdens deze controles. Een afzonderlijke, herkenbare technische supportvraag naar de eigen mailbox `info@paskluis.nl` leverde exact één automatische ontvangstbevestiging en één mailopdracht op. De mailserver weigerde de aanmelding (`MAIL_EAUTH`); de mail is niet verzonden. De testvraag is gesloten en uitsluitend deze testmail is eerst gepauzeerd (`available_at=infinity`). De eigenaar bevestigde vervolgens `info@paskluis.com`; met `SUPPORT_MAILBOX` zijn SMTP-gebruikersnaam, afzender en standaard medewerkersontvanger gecorrigeerd zonder het wachtwoord uit te lezen. De bestaande testvraag/contactgegevens zijn gecorrigeerd en een hervatting van dezelfde mailopdracht is gestart. Het definitieve resultaat moet nog worden opgehaald omdat de browsercontrole daarna vastliep. Testgesprek: `02356d3e-4ec0-4885-acc1-9f1abaafe685`.
...
1. **Supportmail:** `SUPPORT_SMTP_JSON` is op 22 september opgeslagen en wordt door de live worker gelezen. De eerste SMTP-aanmelding gebruikte ten onrechte .nl en werd geweigerd (`MAIL_EAUTH`). De eigenaar bevestigde daarna .com en het bestaande wachtwoord. De live instelling `SUPPORT_MAILBOX=info@paskluis.com` heeft voorrang op `user` en `from` in `SUPPORT_SMTP_JSON`; het wachtwoord blijft uitsluitend daar opgeslagen. Verifieer nu de verzendstatus van de hervatte testmail en de ontvangst. De bestaande Auth-SMTP is geen automatisch gedeelde Edge-secret. Standaard ontvangt `info@paskluis.com` medewerkersmeldingen; `SUPPORT_INBOX_EMAIL` kan dit wijzigen. Voer wachtwoorden en sleutels uitsluitend in de beveiligde beheeromgeving in.
