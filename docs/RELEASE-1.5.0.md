...
Status 22 september 2026: appkandidaten voor iOS en Android geslaagd. Vernieuwd beheer, database en serverfuncties zijn gepubliceerd. Nog geen vrijgave voor de officiële externe test of openbare lancering: storekoppelingen, privacygegevens en toesteltests moeten worden afgerond. Supportmail naar de medewerkersmailbox is succesvol ontvangen en door de eigenaar bevestigd.
...
- Databasewachtrij voor deel- en supportmeldingen, aparte push- en mailstatus, herhaalpogingen, generieke melding zonder kaartcode/PIN/gesprekstekst. Worker en automatische herhaalpogingen zijn actief. De aparte support-SMTP is opgeslagen. Het aanvankelijk onjuiste .nl-mailboxadres gaf `MAIL_EAUTH`; na eigenaarcorrectie gebruikt `SUPPORT_MAILBOX=info@paskluis.com` de bestaande opgeslagen SMTP-password met de juiste gebruikersnaam en afzender. Na hervatting bevestigt de screenshot van de eigenaar de ontvangen mail: 22 september 2026 om 16:16, onderwerp “Nieuwe klantvraag in PasKluis”, afzender PasKluis <info@paskluis.com>, ontvanger info@paskluis.com. De PasKluis-opmaak en knop “Open beheer” zijn zichtbaar. Klantantwoordmail en push op echte telefoons moeten nog worden getest.
...
- Veertien tests van de echte serverhandlers met gesimuleerde aanbieders geslaagd: onafhankelijke push/mailkanalen, herhaalpogingen, ingetrokken deelrechten, geverifieerde terugbetalingen, paginering, accountcontrole, storingsgedrag en veilige SMTP-foutcodes zonder wachtwoorden/providerreacties en een mailboxcorrectie die het wachtwoord behoudt en aanmelding/afzender/ontvanger gelijkzet. Dit bewijst geen echte mailbox-, push- of storebezorging.
...
- Live smokecontrole: meldingsworker antwoordt HTTP 200 met lege wachtrij; onbevoegde account-/aankoop-/deelverzoeken worden geweigerd. De locatiefunctie accepteert de publieke app-sleutel en valideert de invoer. Geen echte klantberichten verstuurd tijdens deze controles. Een afzonderlijke, herkenbare technische supportvraag naar de eigen mailbox `info@paskluis.nl` leverde exact één automatische ontvangstbevestiging en één mailopdracht op. De mailserver weigerde aanvankelijk de aanmelding (`MAIL_EAUTH`); die eerste pogingen verstuurden geen mail. De testvraag is gesloten en uitsluitend deze testmail is eerst gepauzeerd (`available_at=infinity`). De eigenaar bevestigde vervolgens `info@paskluis.com`; met `SUPPORT_MAILBOX` zijn SMTP-gebruikersnaam, afzender en standaard medewerkersontvanger gecorrigeerd zonder het wachtwoord uit te lezen. De bestaande testvraag/contactgegevens zijn gecorrigeerd en een hervatting van dezelfde mailopdracht is gestart. Ondanks de vastgelopen browsercontrole is ontvangst daarna door de eigenaar aangetoond met een inboxscreenshot van 22 september 2026, 16:16. De hervatte supportmail is dus daadwerkelijk ontvangen. De uiteindelijke outbox-status is nog niet rechtstreeks teruggelezen. Testgesprek: `02356d3e-4ec0-4885-acc1-9f1abaafe685`.
...
- Tussenversie `e166ae4` is eerder verspreid (iOS #64 / Android #62). De serverblokkade is verholpen en de definitieve interne testdistributie op `e4885da` is hervat. iOS Release #65 was vóór publiceren geannuleerd. Nieuwe workflows: [iOS Release #66](https://codemagic.io/app/69f36f732d8b59f24897933a/build/6ab281da88dc8aaeb5087050) **geslaagd: versie 1.5.0 (99), upload door Apple zonder fouten geaccepteerd**. Beschikbaarheid in TestFlight na verwerking is nog niet bevestigd. [Android Test #63](https://codemagic.io/app/69f36f732d8b59f24897933a/build/6ab281da31d4c516db3a982a) **geslaagd: versie 1.5.0 (74), succesvol gepubliceerd naar Google Play `internal`; de track meldt `completed`**. De officiële externe test blijft afhankelijk van de vrijgavevoorwaarden.
- Live servercontrole ontdekte ontbrekende tabelrechten voor `service_role`. Migratie 028 geeft uitsluitend de door Edge Functions benodigde SELECT/INSERT/UPDATE/DELETE-rechten; geen wijziging aan klantrollen of RLS. De rollback-proef onder de echte serverrol slaagde. **Na expliciet eigenaarakkoord is migratie 028 succesvol toegepast.** De lees-/schrijftest onder de serverrol slaagde ook ná publicatie. Live aanroepen: meldingsworker HTTP 200 met lege wachtrij; locatiecatalogus HTTP 200 voor een onbekend testmerk (zonder betaalde Places-opvraag); storecontrole bereikt de database en geeft uitsluitend `STORE_NOT_CONFIGURED` voor beide ontbrekende providers.
- Camera, biometrie, betalingen, klantantwoordmail, screenshots en push op twee echte telefoons zijn nog niet end-to-end getest. De medewerkersmail bij een nieuwe supportvraag is wel daadwerkelijk ontvangen.

Niet-blokkerende buildwaarschuwingen vóór openbare lancering nalopen: standaard iOS-launchimage; toekomstige UIScene/Swift Package Manager-overgang van de ML Kit-plug-ins; aangekondigde Gradle/AGP/Kotlin-versievereisten.

## Wat nog nodig is voor vrijgave

1. **Supportmail:** `SUPPORT_SMTP_JSON` is op 22 september opgeslagen en wordt door de live worker gelezen. De eerste SMTP-aanmelding gebruikte ten onrechte .nl en werd geweigerd (`MAIL_EAUTH`). De eigenaar bevestigde daarna .com en het bestaande wachtwoord. De live instelling `SUPPORT_MAILBOX=info@paskluis.com` heeft voorrang op `user` en `from` in `SUPPORT_SMTP_JSON`; het wachtwoord blijft uitsluitend daar opgeslagen. De eigenaar heeft de ontvangen medewerkersmail met een inboxscreenshot bevestigd (22 september, 16:16). Deze SMTP-/ontvangstcontrole is geslaagd. Test nog een antwoord vanuit het beheer naar een gast/gebruiker, inclusief mail en push. De bestaande Auth-SMTP is geen automatisch gedeelde Edge-secret. Standaard ontvangt `info@paskluis.com` medewerkersmeldingen; `SUPPORT_INBOX_EMAIL` kan dit wijzigen. Voer wachtwoorden en sleutels uitsluitend in de beveiligde beheeromgeving in.
2. **Storeaankopen:** configureer `APPLE_IAP_KEY_JSON` (`{privateKey,keyId,issuerId}`) en `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` met de benodigde PasKluis-rechten. Richt in beide stores het eenmalige, niet-verbruikbare product `paskluis_plus` in op € 1,99. `ALLOW_SANDBOX_PURCHASES=true` alleen bewust voor storetesters. De koopknop blijft nu uit. Activeer `store_purchase_enabled` eerst uitsluitend tijdens een afgebakende interne test met correct ingestelde sandbox-/licentietestaccounts, nadat de providers zijn gekoppeld; controleer kopen, annuleren, herstellen en accountbinding vóór publieke verkoop.
...
6. **Gastgesprek herstellen:** bij verlies van de lokale toegangssleutel ontbreekt nog zelfstandig herstel via e-mail. Voor de eerste test blijft contact via `info@paskluis.com` de uitwijkmogelijkheid. Rechtstreeks per e-mail antwoorden met automatische terugkoppeling naar het appgesprek is bewust uitgesteld.
...
| Support antwoord, mail, push, storingen | Correct gesprek opent; geen interne notities in klantdata | Medewerkersmail ontvangen; klantantwoord/push nog testen |
